class VinosController < ApplicationController
  ...
  # POST /vinos.json
  def create
    @vino = Vino.create(name: params[:vino][:name])
    cepas = params[:vino][:cepa_ids].select(&:present?).map(&:to_i)
    porcentajes = params[:vino][:porcentaje]
    cepas.each_with_index do |cepa, index|
      @ensamblaje = Ensamblaje.create!(vino_id: @vino.id, cepa_id: cepa,  porcentaje: porcentajes[index])
    end
